...
SQL_MODE_HIGH_NOT_PRECEDENCE  = 2
SQL_MODE_PIPES_AS_CONCAT      = 4
SQL_MODE_IGNORE_SPACE         = 8
SQL_MODE_NO_BACKSLASH_ESCAPES = 16
...
# def determine_function(ctx, proposed): return proposed    # maybe should return Identifier
# def check_null(text) return NULL2_SYMBOL if text == r"\N" else ANTLR3_TOKEN_INVALID
...
join_table: // Like the same named rule in sql_yacc.yy but with removed left recursion.
	(INNER_SYMBOL | CROSS_SYMBOL)? JOIN_SYMBOL table_reference (join_condition)?
	| STRAIGHT_JOIN_SYMBOL table_factor (ON_SYMBOL expression)?
...
other_administrative_statement:
	...
	| KILL_SYMBOL  ((CONNECTION_SYMBOL | QUERY_SYMBOL))? expression
...
expression_list_with_direction:
	expression direction? (COMMA_SYMBOL expression direction?)*
;
...
AT_SIGN_SYMBOL:				'@'; // MC ((SIMPLE_IDENTIFIER) => SIMPLE_IDENTIFIER /*{ $type = AT_TEXT_SUFFIX; }*/ )?;
...
UNDERSCORE_CHARSET:	UNDERLINE_SYMBOL IDENTIFIER; // MC /*{ $type = check_charset(PAYLOAD, $text); }*/;
...
// Number types.
NUMBER: DIGITS ; // MC /*{ $type = determine_num_type($text); }*/;
...
DECIMAL_NUMBER:
	...
	// MC | DOT_SYMBOL {if (!isAllDigits(ctx)) {FAILEDFLAG = ANTLR3_TRUE; return; }} DIGITS
    //;
...
/* MC all quote lexers, my head is going to explode
BACK_TICK_QUOTED_ID
...
fragment VERSION_COMMENT_TAIL:
	{ VERSION_MATCHED == ANTLR3_FALSE }? // One level of block comment nesting is allowed for version comments.
		((ML_COMMENT_HEAD MULTILINE_COMMENT) | . )*? ML_COMMENT_END { $type = MULTILINE_COMMENT; $channel = HIDDEN; }
...
fragment MULTILINE_COMMENT:	(.*? ML_COMMENT_END) -> channel(HIDDEN) ;
...
// MC made case sensitive
fragment LETTER_WHEN_UNQUOTED_NO_DIGIT:
	('A'..'Z'|'a'..'z') // Only upper case, as we use a case insensitive parser (insensitive only for ASCII).
	| '$'
	| '_'
	| '\u0080'..'\uffff'
;
